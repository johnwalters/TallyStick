...
| Field | Current value |
| --- | --- |
| Overall status | Phases 0–5 complete through the independently approved Slice 13/CF5-GATE; Phase 6 remains deferred |
| Current phase | Phase 5 complete — Investing, Financing, transfers, and disclosures |
| Next task | Next eligible but not authorized: `CF-SLICE-14-RECONCILIATION-WARNINGS`; do not start Slice 14 or authorize later Phase 6+ work |
...
| Last verified Cash Flow test | Slice 13 focused suite: 52/52; canonical fixture/oracle gate: 20 static scenarios plus 52 production acceptance tests; `npm run test:ci`: 290/290 browser/unit tests plus dependency boundaries and both fixture oracles (Balance Sheet 2 companies/19 scenarios; Cash Flow 2 companies/20 scenarios); `npm run build` (1.41 MB, no warnings); app/spec TypeScript, boundary, scoped diff, and byte-identical oracle-copy checks passed. Desktop/native smoke was not applicable because Slice 13 crossed no native or persistence boundary. |
| Last tracker update | August 27, 2026 — `CF-SLICE-13-OPENING-NONCASH` and CF5-GATE independently approved; Phase 5 complete |
| Known blocker | None for completed Phase 5. Phase 6+ slices remain intentionally deferred and are not authorized. |
...
- [x] **CF2-008** Implement classification repository mappings, atomic batch updates, and revision increments.
...
- [x] **CF2-011** Update backup, restore, portable export, Electron validation, temporary databases, and smoke fixtures through the shared schema constant.
- [x] **CF2-012** Test schema-6-to-7 migration, idempotent reopen, corrupt migration rollback, backup/restore, relocation, and future-schema rejection.
...
- [x] **CF2-GATE** Exit gate: schema-6 data safely reopens as schema 7, recovery paths pass, and no existing ledger or statement value drifts.
...
- [x] **CF5-005** Eliminate confirmed cash-to-cash transfers from all activity sections and Net Change.
- [x] **CF5-006** Handle transfers between unrestricted and restricted cash without double counting and expose the required presentation/warning.
- [x] **CF5-007** Identify supported noncash investing/financing events, exclude them from cash totals, and disclose them separately.
- [x] **CF5-008** Handle in-period opening-balance events per the explicit opening-balance source/treatment rules.
- [x] **CF5-009** Build deterministic Investing and Financing hierarchies, subtotals, signs, zero handling, and exact detail for Slice 11; complete the supplemental-disclosure hierarchy with deterministic review grouping, warnings, and exact detail.
- [x] **CF5-010** Test the Slice 11 fixed-asset, debt, mixed-payment, owner-equity, and supported cash-side transfer cases; Slice 12 transfer/restricted-cash cases; and Slice 13 noncash acquisitions, opening events, hierarchy diagnostics, zero behavior, and exact disclosure/reconciliation cases.
- [x] **CF5-GATE** Exit gate: exact A6 and A8–A13 expectations pass with no duplicated cash or inferred accounting treatment, proven by the executable canonical fixture/oracle gate.
